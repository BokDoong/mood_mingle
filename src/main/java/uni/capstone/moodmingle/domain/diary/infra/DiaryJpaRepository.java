package uni.capstone.moodmingle.domain.diary.infra;
...
public class DiaryJpaRepository implements DiaryRepository {
    ...
    @Override
    public List<Diary.Emotion> findMonthlyEmotionsInfo(Long memberId, LocalDate date) {
        LocalDate startDate = date.withDayOfMonth(1); // 해당 월의 시작일
        LocalDate endDate = date.withDayOfMonth(date.lengthOfMonth()); // 해당 월의 마지막 날
        ...
        return em.createQuery(
                        "select d.emotion" +
                                ...
                                " where m.id = :memberId" +
                                " and d.date >= :startDate and d.date <= :endDate" +
                                " order by d.date asc", Diary.Emotion.class)
                .setParameter("memberId", memberId)
                .setParameter("startDate", startDate)
                .setParameter("endDate", endDate)
                .getResultList();
