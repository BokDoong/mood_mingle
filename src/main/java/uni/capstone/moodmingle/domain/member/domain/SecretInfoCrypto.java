...
import uni.capstone.moodmingle.domain.member.exception.MemberKeyCryptoException;
...
public class SecretInfoCrypto {
    ...
    private byte[] encrypt(byte[] value) {
        try {
            ...
            throw new MemberKeyCryptoException(ErrorCode.FAILED_ENCODING_DATA, e.getMessage());
        }
    }
    ...
    private byte[] decrypt(byte[] encryptedSecretInfo) {
        try {
            ...
            throw new MemberKeyCryptoException(ErrorCode.FAILED_DECODING_DATA, e.getMessage());
        }
    }
