...
public enum ErrorCode {
    ...
    MISSING_REQUESTED_DATA(CommonCode.MISSING_REQUEST_DATA.getCode(), BAD_REQUEST, "요청에서 빠진 파라미터 혹은 데이터가 있을 때"),
    NONE_REQUESTED_URI(CommonCode.NONE_REQUEST_URI.getCode(), BAD_REQUEST, "요청한 URI 가 잘못된 경우"),
    TOO_MANY_REQUESTS(CommonCode.TOO_MANY_REQUESTS.getCode(), HttpStatus.TOO_MANY_REQUESTS, "요청 횟수가 초과된 경우"),
    ...
    FAILED_ENCODING_DIARY(DiaryCode.FAILED_ENCODING.getCode(), NOT_IMPLEMENTED, "알기 및 답변의 인코딩이 실패한 경우"),
    FAILED_DECODING_DIARY(DiaryCode.FAILED_DECODING.getCode(), NOT_IMPLEMENTED, "알기 및 답변의 디코딩이 실패한 경우")
    ;
